...
- 如果当前环境启用的是 `text-global` 文本 provider，且实际可用模型是 `gpt-5.4 / claude-sonnet-4-6` 这类全局模型，营销日历也允许直接复用，不再默认要求必须先有 `deepseek/kimi/doubao`
- 如果 provider 的 `modelWhitelist` 暂时没维护完整，营销日历还会继续读取 provider 的 `defaultModel` 作为候选模型，避免因为白名单缺项而误报“营销日历模型配置读取失败”
