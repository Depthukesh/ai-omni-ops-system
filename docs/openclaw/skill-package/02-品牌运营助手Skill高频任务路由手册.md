...
- 当用户继续追问“OpenChatCut 里 Agent / 生图 / TTS / 转写 / 视频 / 音乐该怎么接本站”时，下一跳应改为 `个人中心 -> OpenClaw 安装中心`：
  - 那里已经有统一网关地址
  - 也有 A / B / C 三类配置示例
  - 还有 curl 测试命令
- 不要把“素材桥接草案”和“统一模型网关配置”混成同一件事：
  - 桥接工具负责整理素材和时间线草案
  - 安装中心负责告诉用户 OpenChatCut 自定义接口怎么填
...
- OpenChatCut 统一网关、A / B / C 配置示例和 curl 测试命令查看
- 后台管理台
