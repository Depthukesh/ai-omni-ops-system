...
- 安装链路已完成
- 核心执行类 MCP 已覆盖网站大部分主链路
- 技能配置治理类 MCP 已接入
- 反馈与提示词优化闭环尚未完成
- 品牌运营助手 Skill 已进入正式交付结构，但仍应继续围绕 MCP 闭环迭代
...
- 当前真正需要安装的是网站 MCP
- 页面里的“品牌运营助手 Skill”已经可以通过安装中心下载正式 ZIP 包
- 正式 Skill 不应复制网站功能，而应通过外部文档 + MCP 调用网站能力
- 昨日新增的 `多元探索 / duoyuanx` 已进入网站供应商目录和第三方接口配置域；当前 Skill/MCP 公共能力面已补入口径，默认把它视为统一网关型第三方平台，而不是单独工作台
...
- 第三方接口配置摘要、品牌共享 Key 更新、运行时可用性检查
...
其中，`多元探索 / duoyuanx` 现在应按以下统一口径处理：

- 它属于“第三方接口配置”域，不属于独立业务工作台
- 当前公共能力面默认通过以下工具处理：
  - `list_my_third_party_platforms`
  - `check_my_third_party_platform_runtime_access`
  - `update_my_third_party_platform_secret`
- 当前已预装文本、图像、视频、音频、音乐五类运行时
- Skill 在遇到“多元探索现在能不能用”“OpenClaw 是否还能继续追问密钥”“这份品牌共享 Key 能不能给五类能力共用”这类问题时，应先查运行时可用性，再决定是否更新密钥或路由到业务工具
...
当前正式 Skill 已可交付，但它的角色仍然应定位为“网站能力总调度 + 路由与执行手册”，而不是脱离网站单独生长的一套平行系统。

所以当前顺序应该是：

1. 持续补齐 MCP 与网站功能目录
2. 让正式 Skill 始终引用同一套 MCP 与外部手册
3. 再逐步补反馈分析与提示词优化闭环

### 后续正式 Skill 的定位

正式 Skill 应该继续保持为一个总入口的“品牌运营助手 Skill”，而不是拆成很多复制网站功能的小 Skill。
