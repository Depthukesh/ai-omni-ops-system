...
- 个人中心已接入第一版真实多用户登录态，并已落地概览页、`/orders`、`/tasks`、`/team`、`/invites` 五段前端路由；其中 `orders` 已支持用户级订单查询、状态/类型筛选与订单详情跳转，`team` 已支持成员添加、角色/状态修改、创建邀请、撤回邀请、接受邀请、邀请码加入、邀请链接复制、成员审计日志查看和主账号转移入口；用户技能覆盖层和更细的任务中心能力仍待继续升级
