...
- 生成品牌增长报告
  ...
  - 当前已开始接入知识库“接入对象”运行时链路：会先按 `bindingType=MODULE`、`targetId=brand-growth-workbench` 查启用中的知识绑定，再从企业知识库召回片段，作为品牌增长报告的补充上下文一起送入模型
...
- 知识库管理
  ...
  - 当前“接入对象”处于“治理层已落地、运行时部分生效”状态：
    ...
    - 已生效：品牌增长报告开始按 `brand-growth-workbench` 自动召回企业知识库片段
    - 未完全生效：其余能力包、提示词、工作流步骤尚未统一接入同一套运行时知识注入链
