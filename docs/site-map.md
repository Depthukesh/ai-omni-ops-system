...
- 原创创作
  ...
  - 文章阶段新增显式 `执行文章AI` 动作，调用 `公众号-创作文章` 对应的第三方文本模型生成标题、摘要、正文和 HTML 草稿
  - 生图阶段会分别调用 `公众号-封面图生成` 与 `公众号-正文配图生成` 对应的第三方图片模型，返回真实生成图片 URL
...
- 视频号/私域尚未独立落地
- 公众号已独立落地正式工作台，文本生成、生图和一键发布均已接入真实第三方接口；当前剩余工作集中在持久化与联调收口
