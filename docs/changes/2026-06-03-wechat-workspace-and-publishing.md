...
- 当前公众号工作台已接入真实文本模型、真实文生图模型与真实公众号官方 API 草稿发布链路。
- 当前剩余主要收口项为：
  - 历史记录与工作流运行态仍以 mock store 为主，尚未完全切到正式持久化
  - 真实账号联调与多账号场景仍需补一轮完整验收
...
- 用户进一步确认：当前公众号创作文章板块并未按外部参考公众号技能文档的多步骤工作流开发。
- 用户提供的技能文档包含：
  - `SKILL.md`
  - `references/`
  - `scripts/`
...
## 2026-06-06 真实第三方模型与公众号 API 接入

### 本次实现

- 公众号工作流 `Step 1 输入` 当前先保存原始输入资料。
- 公众号工作流 `Step 2 文章` 新增显式 `执行文章AI` 动作，真实调用第三方文本大模型生成标题、摘要、正文和 HTML 草稿，不再把模型调用隐藏在输入保存动作里。
- 运行时会读取技能中心中 `公众号-创作文章` 当前绑定的模型与 Provider 偏好，并沿用统一的文本模型 fallback 规则。
- 公众号工作流 `Step 3 生图` 已改为真实调用第三方图片模型，不再仅返回拼装的 `text_to_image` 演示 URL。
- 封面图与正文配图分别读取：
  - `公众号-封面图生成`
  - `公众号-正文配图生成`
  两个技能在技能中心中的当前模型配置。
...
- 本轮补充了公众号 HTML 与生图结果的自动同步策略：
  - 保持“先文章、后生图”的既有步骤顺序不变
  - 生图完成后自动把封面图与正文配图 URL 回写到现有 `htmlContent`
  - 若文章 HTML 中已有 `<img>` 占位图，则按顺序替换为真实生成结果
  - 若文章 HTML 中没有图片占位，则在 HTML 尾部自动追加封面图与正文配图区块
- 工作流预览与历史草稿预览都已加入旧数据兜底：即使历史记录中的 `htmlContent` 尚未持久化写回图片 URL，前端打开 HTML 预览时也会基于当前图片任务结果临时补图，避免再次出现“图片已生成但预览仍是裂图/空图”。
- 页面中的正文输入框仍保留纯文本编辑体验，但增加说明：该区域主要用于人工修订内容文本，真正预览与发布以 HTML 排版结果为准。
- 生图阶段已改为优先使用文章阶段产出的：
  - `coverImageBrief`
  - `bodyImageBriefs`
  作为封面图和正文配图提示词来源，不再完全依赖固定英文模板 prompt。
- 公众号工作流中的封面图、正文配图和发布历史封面图现在都支持点击弹出站内预览窗口，便于在发布前检查细节，而不是仅静态展示缩略图。
- 当前如果第三方图片接口返回 `502/503/504/524`，系统会明确显示为上游图片接口异常，提示用户稍后重试或切换图片模型/供应商。
- 公众号工作流生图当前已从“正文配图并发触发”调整为“逐张串行错峰触发”，避免同一轮同时向第三方平台压出多张任务，降低 `rate_limit_exceeded / downstream service error` 的概率。
- 生图阶段当前改为“至少有一张成功就保留结果”：
  - 只有封面图和正文配图全部失败、没有任何一张图片生成成功时，才将整轮生图判定为失败
  - 若部分图片成功，则先保留成功结果并回写到当前 `htmlContent`
  - 若仍有未成功图片，则工作流停留在生图阶段，页面展示“已成功生成 X/Y 张图片，可继续重试补齐”，用户可直接再次生成
- 公众号“发布历史 / 历史草稿”当前已改成更接近小红书原创笔记的作品卡片式展示：
  - 封面直接展示在卡片顶部
  - 状态、`media_id`、重试次数等信息以下方徽标和卡片操作区呈现
  - 发布记录支持卡片化查看详情与重试发布，历史草稿支持卡片化一键发布与查看 HTML

### 2026-06-06 第三轮收口

- 公众号文章生成提示词与 HTML 包装层进一步收口：
  - 明确要求正文配图以内嵌 `<img>` / `<figure><img /></figure>` 占位形式出现在对应章节中
  - 明确禁止在文末追加“营销日历资料 / 产品资料 / 品牌资料 / 原文链接 / 创作来源 / 素材说明 / 附录”等尾部信息块
  - 后端工作流 HTML 与草稿 HTML 的兜底包装层已同步移除上述文末资料块
- 公众号工作流 `Step 1 输入` 与新建工作流表单新增“正文配图尺寸”下拉：
  - 当前支持 `横版 4:3 / 横版 16:9 / 方图 1:1 / 竖版 3:4`
  - 封面图尺寸固定为 `1600x900`
  - `WechatWorkflowSession.bodyImageSize` 已进入 Prisma schema 与迁移，避免只停留在运行时内存状态
...
- 后端：
  - `apps/server/src/modules/works/works.service.ts`
  - `apps/server/src/modules/works/wechat-official-account-api.service.ts`
- 前端：
  - `apps/web/src/app/(dashboard)/wechat/workspace-shell.tsx`
  - `apps/web/src/services/works.ts`
- 文档：
