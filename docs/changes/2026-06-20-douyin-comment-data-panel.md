...
- 强制校验输入链接和评论结果中的 `sec_user_id`，不满足要求的数据不入库、不展示。
...
- 强制校验 `sec_user_id`，是因为评论洞察后续往往要按用户维度继续分析，没有 `sec_user_id` 的记录价值不足。
...
- 手工验证：
  ...
  - 输入带 `sec_user_id` 的抖音作品链接，点击提交后能出现评论结果列表。
  - 输入缺少 `sec_user_id` 的链接时，前后端会给出错误提示。
- 接口验证：
...
- 当前输入链接强依赖能解析到 `aweme_id` 和 `sec_user_id`，如果分享链路格式变化，需要继续补充链接解析兼容。
