...
- 当前业务明确要求评论结果里必须保留 `sec_user_id`，其中前端重点展示评论用户 `sec_user_id`，便于后续用户分析。
...
- 新增评论数据结果表格，展示核心字段：
  ...
  - `replyCount`
  - `collectedAt`
- 评论结果区新增“加载更多”，按 TikHub 评论接口的 `cursor` 继续翻页拉取更多评论，并把新结果追加进当前工作区列表。
...
- 手工验证：
  ...
  - 当接口返回 `has_more=true` 时，点击“加载更多”能够继续拉取下一批评论并追加到现有列表。
- 接口验证：
...
- 当前评论数据已支持按 `cursor` 继续翻页；如果后续需要页码感更强的交互，可以再补“第 N 页”包装层。
