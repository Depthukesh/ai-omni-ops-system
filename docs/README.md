...
- 文件：`docs/production-web-port-conflict-recovery.md`
- 作用：沉淀线上前端 `3001` 端口冲突、`root` 残留 `next-server` 占端口、`aiops + pm2` 接管失败等生产排障步骤，作为部署后页面仍是旧版或白屏时的第一入口
- 更新时机：当生产部署链、前端运行方式、PM2 管理规则、端口占用排障流程发生变化时
...
- 文件：`docs/project_planning/61_AI全域运营系统_全站视觉统一改造计划_v1.md`
- 作用：记录全站页面如何在不破坏现有业务链路的前提下，按“首页、认证、工作台、个人中心、后台”分阶段统一视觉语言、公共壳层和基础组件
- 更新时机：当全站视觉统一的阶段划分、优先级、设计系统边界或验收标准发生变化时
...
- `docs/changes/2026-06-10-knowledge-binding-runtime-and-docs-sync.md`
  - 知识绑定运行时已扩到品牌增长报告、营销策划、抖音热点找选题、公众号工作流的文章生成 / 配图生成 / HTML 渲染、小红书原创 / 二创文案与配图提示、小红书视频笔记文案 / 提示词 / 故事板、抖音视频阶段技能、抖音数字人口播脚本，以及设计工作台的图片 / HTML / PPT / 视频方案；顶层文档同步标注治理层与运行时边界
