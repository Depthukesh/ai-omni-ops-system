...
flowchart TD
    ...
    H1 --> H18["standard 运行态默认展示仓库版本记录 + 安装前软件依赖 + Docker 安装/更新命令；配置 STANDARD_RUNTIME_UPDATE_MANIFEST_URL 后再叠加远端更新清单与新版本提醒"]
    ...
    B --> B1["某书：营销策划方案（OpenClaw HTML）/ 创作素材 / 每日计划 / 每周复盘 / 策略优化记录 / 作品列表"]
    B --> B2["某音/某号：营销策划方案（OpenClaw HTML）/ 数字人 / RunningHub应用 / 创作素材 / 每日计划 / 每周复盘 / 策略优化记录 / 作品列表"]
    B --> B3["公众号：营销策划方案（位于配置初始化上方）/ 配置初始化 / 创作工作流 / 发布历史 / 创作素材 / 每日计划 / 每周复盘 / 策略优化记录 / 作品列表"]
    B --> B4["OpenClaw 六类内容详情支持留言；每周复盘与策略优化记录支持编辑"]
