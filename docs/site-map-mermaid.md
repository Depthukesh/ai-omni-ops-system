...
flowchart LR
    ...
    Personal --> PersonalVideoRemix["/personal-center/third-party-platforms/video-remix"]
...
flowchart TD
    ...
    C6 --> C61["内容获客直接承载 mixedcut /remix 主界面；模型同步收口到个人中心视频混剪设置"]
    C6 --> C63["工作区支持手动勾选站内视频，由主站后端上传到 mixedcut 并直接创建任务；完成后自动回流到某音/某号统一作品列表"]
    C6 --> C62["mixedcut 容器根路由收口到 /remix，健康检查统一走 /api/health"]
    C6 --> C64["OpenClaw MCP 已补 mixedcut 工具；支持站内视频 / 创作素材 / 本机 localFilePath 归档后发起混剪"]
    C6 --> C65["mixedcut 已同步到 MCP 与 Skill ZIP；Skill 已补视频混剪、本机路径直通与站内素材复用语义"]
    C6 --> C66["mixedcut 最近项目区已去掉前端模板里的 5 条 / 20 条截断"]

    B --> B1["某书：营销策划方案 / 创作素材 / 每日计划 / 每日复盘 / 作品列表"]
    B --> B2["某音/某号：营销策划方案 / 数字人 / RunningHub应用 / 视频混剪 / 创作素材 / 每日计划 / 每日复盘 / 作品列表"]
    ...
    B --> B7["站内素材已支持最小桥接：手动选择 -> 主站上传 -> mixedcut 创建任务；仍不是自动同步"]
    ...
    H6 --> H63["视频混剪不再单独配平台；直接复用品牌已有第三方模型配置做 mixedcut 同步"]
    H6 --> H64["/personal-center/third-party-platforms/video-remix 单独承接 ai_config.json 预览与同步"]
    H6 --> H65["同模块同时承接 mixedcut 素材桥接接口：media-assets / remix-task / progress"]
