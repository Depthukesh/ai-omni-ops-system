...
- `local-single-user` 安装态个人中心已开始承接“版本与升级”，默认通过 OSS `latest.json` 检查新包，并由后端受控触发独立 updater 执行本地替换；标准运行态只有在显式配置远端更新清单后才展示该入口，且只提供更新通知与命令引导，不直接替用户升级容器；网站版或未配置清单的源码运行态默认不展示该入口
