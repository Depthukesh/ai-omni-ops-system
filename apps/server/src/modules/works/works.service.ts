...
const WECHAT_IMAGE_MODEL_ATTEMPT_TIMEOUT_MS = 240 * 1000;
...
export type UpdateDouyinDirectVideoPayload = {
  title?: string;
  content?: string;
};
...
const wechatWorkflowSessionMockStore: WechatWorkflowSessionStoreItem[] = [
  {
    ...
    imageBundle: {
      status: "SUCCESS",
      promptSummary: "围绕会员日主题生成封面图与正文插图，风格温暖、门店感明显。",
      ...
      bodyImageUrls: [
        "https://coresg-normal.trae.ai/api/ide/v1/text_to_image?prompt=" +
          encodeURIComponent("bakery store campaign illustration for wechat article body, summer membership event, pastry display, warm light") +
          "&image_size=landscape_16_9",
        "https://coresg-normal.trae.ai/api/ide/v1/text_to_image?prompt=" +
          encodeURIComponent("close-up pastry product visual for wechat article body, tiramisu cake, premium bakery, elegant composition") +
          "&image_size=landscape_16_9",
      ],
      prompts: ["会员日封面图", "门店场景插图", "主推产品插图"],
    },
    publishConfig: {
      ready: true,
      accountId: "wechat_account_demo_001",
      ...
      commentMode: "open",
      fanCommentsOnly: false,
      checklist: ["已配置 AppID / AppSecret", "已配置 IP 白名单", "已生成封面图", "已确认评论策略"],
    },
...
export class WorksService {
  ...
  private async resolveBrandAwareApiKeys(
    ...
    if (resolution.status === "brand-api-key-missing") {
      ...
      throw new ServiceUnavailableException(
        `${prefix}当前品牌尚未配置第三方平台「${resolution.platform.name}」API Key，请先前往个人中心-第三方接口配置完成品牌共享设置后再试。`,
      );
    }
  ...
  async listDouyinDirectVideoProviderOptions() {
    return this.listXiaohongshuVideoProviderOptions();
  }
  ...
  async generateWechatWorkflowImages(brandId: string, workflowId: string, auth?: RequestAuthContext) {
    ...
    const task = await this.createOriginalTask({
      userId: await this.resolveTaskUserId(brandId, auth),
      brandId,
      taskType: "WECHAT_IMAGE_AI",
      taskTitle: `生成公众号工作流图片：${target.title}`,
    ...
    await this.updateTaskRunningOutput(task.id, {
      ...
      totalCount: prompts.length,
      failedCount: 0,
      coverModelName: coverImageConfig.providers[0]?.models[0] || "",
      bodyModelName: bodyImageConfig.providers[0]?.models[0] || "",
    }, {
      modelName: coverImageConfig.providers[0]?.models[0] || bodyImageConfig.providers[0]?.models[0] || "gpt-image-2",
    });
    setTimeout(() => {
  ...
  private async runWechatWorkflowImagesTask(
    brandId: string,
    workflowId: string,
    taskId: string,
    promptSummary: string,
    ...
    try {
      ...
      try {
        ...
        await this.updateTaskRunningOutput(taskId, this.buildWechatImageTaskOutput(target, {
          promptSummary,
          prompts,
          coverAsset,
          bodyAssets,
        ...
        await this.updateTaskRunningOutput(taskId, this.buildWechatImageTaskOutput(target, {
          ...
          imageFailures,
          stage: "GENERATING_IMAGES",
        }), { modelName: latestModelName || coverImageConfig.providers[0]?.models[0] || "gpt-image-2" });
      }
      ...
      for (let index = 0; index < bodyPrompts.length; index += 1) {
        ...
        await this.updateTaskRunningOutput(taskId, this.buildWechatImageTaskOutput(target, {
          ...
          imageFailures,
          stage: "GENERATING_IMAGES",
        }), { modelName: latestModelName || bodyImageConfig.providers[0]?.models[0] || coverImageConfig.providers[0]?.models[0] || "gpt-image-2" });
      }
      ...
      target.imageBundle = {
        ...
        taskId,
        promptSummary,
        startedAt: target.imageBundle?.startedAt,
        generatedAt: now,
        lastGeneratedAt: now,
      ...
      await this.persistWechatWorkflowSessionStoreItem(target);
      await this.markTaskSuccess(taskId, this.buildWechatImageTaskOutput(target, {
        promptSummary,
        prompts,
        coverAsset,
        bodyAssets,
        imageFailures,
        stage: "GENERATED_IMAGES",
      }), {
        modelName: latestModelName || coverAsset?.modelName || bodyAssets[0]?.modelName || coverImageConfig.providers[0]?.models[0] || "gpt-image-2",
      });
    } catch (error) {
      ...
      if (generatedImageCount > 0) {
        const now = new Date().toISOString();
        const partialDetail = this.buildWechatPartialImageGenerationNotice(
          prompts.length,
          generatedImageCount,
          [...imageFailures, message],
        );
        target.imageBundle = {
          status: "SUCCESS",
          taskId,
          ...
          generatedCount: generatedImageCount,
          totalCount: prompts.length,
          failedCount: imageFailures.length + 1,
          coverProvider: coverAsset?.providerName,
          coverRuntimeKey: coverAsset ? "image-generation" : undefined,
          coverModelName: coverAsset?.modelName,
          bodyProvider: bodyAssets[0]?.providerName,
          bodyRuntimeKey: bodyAssets.length ? "image-generation" : undefined,
          bodyModelName: bodyAssets[0]?.modelName,
          errorDetail: partialDetail,
        };
        target.publishConfig = undefined;
        target.status = "IMAGE_PENDING";
        target.currentStep = "image";
        target.errorDetail = partialDetail;
        target.updatedAt = now;
        await this.persistWechatWorkflowSessionStoreItem(target);
        await this.markTaskSuccess(taskId, this.buildWechatImageTaskOutput(target, {
          promptSummary,
          ...
          coverAsset,
          bodyAssets,
          imageFailures: [...imageFailures, message],
          stage: "GENERATING_IMAGES_PARTIAL",
        }), {
          modelName: latestModelName || coverAsset?.modelName || bodyAssets[0]?.modelName || coverImageConfig.providers[0]?.models[0] || "gpt-image-2",
        });
        return;
      ...
      target.imageBundle = {
        ...
        generatedCount: 0,
        totalCount: prompts.length,
        failedCount: imageFailures.length || prompts.length,
        errorDetail: message,
      };
      ...
      await this.markTaskFailed(taskId, message, {
        modelName: latestModelName || coverImageConfig.providers[0]?.models[0] || bodyImageConfig.providers[0]?.models[0] || "gpt-image-2",
        outputJson: this.buildWechatImageTaskOutput(target, {
          promptSummary,
          prompts,
          coverAsset,
          bodyAssets,
          imageFailures: imageFailures.length ? imageFailures : [message],
          stage: "GENERATING_IMAGES_FAILED",
        }),
      });
    }
  }

  private assertWechatImageTaskWithinDeadline(deadlineMs: number) {
    if (Date.now() > deadlineMs) {
      throw new ServiceUnavailableException(`公众号生图总耗时超过 ${Math.round(WECHAT_IMAGE_TASK_TOTAL_TIMEOUT_MS / 60000)} 分钟，请重试或切换兜底模型。`);
    }
  }

  private async persistWechatWorkflowImageProgress(
    target: WechatWorkflowSessionStoreItem,
    params: {
      promptSummary: string;
      prompts: string[];
      coverAsset: Awaited<ReturnType<WorksService["generateImageAsset"]>> | null;
      bodyAssets: Array<Awaited<ReturnType<WorksService["generateImageAsset"]>>>;
      imageFailures: string[];
      taskId: string;
    },
  ) {
    const generatedCount = (params.coverAsset ? 1 : 0) + params.bodyAssets.length;
    const now = new Date().toISOString();
    target.imageBundle = {
      status: "RUNNING",
      ...
      lastGeneratedAt: generatedCount ? now : target.imageBundle?.lastGeneratedAt,
      coverImageUrl: params.coverAsset?.url,
      bodyImageUrls: params.bodyAssets.map((item) => item.url),
      prompts: params.prompts,
      generatedCount,
      totalCount: params.prompts.length,
      failedCount: params.imageFailures.length,
      coverProvider: params.coverAsset?.providerName,
      coverRuntimeKey: params.coverAsset ? "image-generation" : undefined,
      coverModelName: params.coverAsset?.modelName,
      bodyProvider: params.bodyAssets[0]?.providerName,
      bodyRuntimeKey: params.bodyAssets.length ? "image-generation" : undefined,
      bodyModelName: params.bodyAssets[0]?.modelName,
      errorDetail: params.imageFailures.length
        ? this.buildWechatPartialImageGenerationNotice(params.prompts.length, generatedCount, params.imageFailures)
        : undefined,
    };
    target.publishConfig = undefined;
    target.status = "IMAGE_PENDING";
    target.currentStep = "image";
    target.errorDetail = target.imageBundle.errorDetail;
    target.updatedAt = now;
    await this.persistWechatWorkflowSessionStoreItem(target);
  }

  private buildWechatImageTaskOutput(
    target: WechatWorkflowSessionStoreItem,
    params: {
      promptSummary: string;
      prompts: string[];
    ...
    return {
      ...
      promptSummary: params.promptSummary,
      totalCount: params.prompts.length,
      generatedCount: (params.coverAsset ? 1 : 0) + params.bodyAssets.length,
      failedCount: params.imageFailures.length,
      coverImageUrl: params.coverAsset?.url || "",
      bodyImageUrls: params.bodyAssets.map((item) => item.url),
      coverModelName: params.coverAsset?.modelName || "",
      ...
      coverProvider: params.coverAsset?.providerName || "",
      bodyProvider: params.bodyAssets[0]?.providerName || "",
      imageFailures: params.imageFailures,
      errorDetail: params.imageFailures.length
        ? this.buildWechatPartialImageGenerationNotice(
          params.prompts.length,
          (params.coverAsset ? 1 : 0) + params.bodyAssets.length,
          params.imageFailures,
        )
        : undefined,
    };
  }
  ...
  async updateWechatWorkflowPublishConfirm(
    ...
    target.author = String(payload.author || "").trim() || target.author;
    target.commentMode = payload.commentMode || target.commentMode;
    const fanCommentsOnly = payload.fanCommentsOnly ?? target.publishConfig?.fanCommentsOnly ?? false;
    const coverImageUrl =
  ...
  async createDouyinDigitalHumanScriptTemplate(
    ...
    const note = this.normalizeDigitalHumanScriptTemplateNote(payload.note);
    const category = this.normalizeDigitalHumanScriptTemplateCategory(payload.category);
    if (!content) {
  ...
  async updateDouyinDigitalHumanScriptTemplate(
    ...
    const hasNote = typeof payload.note !== "undefined";
    ...
    if (!hasName && !hasContent && !hasNote && !hasShared && !hasCategory && !hasArchived) {
    ...
    const normalizedNote = hasNote ? this.normalizeDigitalHumanScriptTemplateNote(payload.note) : undefined;
    const normalizedCategory = hasCategory ? this.normalizeDigitalHumanScriptTemplateCategory(payload.category) : undefined;
  ...
  async generateDouyinDirectVideo(
    ...
    const context = await this.resolveDouyinDirectVideoComposerContext(brandId, payload, collaboratorRole);
    const userId = await this.resolveTaskUserId(brandId, auth);
    const task = await this.createVideoTask({
      userId,
      brandId,
      taskType: "DOUYIN_DIRECT_VIDEO",
      taskTitle: `生成 AI 生视频：${context.topicLabel}`,
      requestedVideoProvider: context.requestedVideoProvider,
      modelName: "deepseek-v4-pro",
    });
    const now = new Date().toISOString();
    const htmlContent = this.renderGeneratedVideoNoteHtml({
      ...
      videoKindLabel: this.getVideoKindLabel(context.videoKind),
      workflowStage: "QUEUED",
      progressSteps: this.buildVideoProgressSteps("QUEUED", "DOUYIN_DIRECT_VIDEO"),
    });
    const htmlFile = await this.writeGeneratedTextFile(brandId, `${task.id}-douyin-direct-video.html`, htmlContent);
    const metadata: VideoWorkAssetMeta = {
      kind: "DOUYIN_DIRECT_VIDEO",
      taskId: task.id,
      noteCategory: "原创",
      noteType: "视频",
      accountRole: context.accountRole,
      ...
      resolvedVideoProvider: context.requestedVideoProvider,
      requestedDurationSec: context.requestedDurationSec,
      requestedAspectRatio: context.requestedAspectRatio,
      progressSteps: this.buildVideoProgressSteps("QUEUED", "DOUYIN_DIRECT_VIDEO"),
      storyboardRevisions: [],
      segmentPrompts: [],
      createdAt: now,
    ...
    const workMedia = await this.createWorkHtmlMedia({
      ...
      brandId,
      taskId: task.id,
      title: `抖音 AI 生视频 - ${context.topicLabel}`,
      storageKey: htmlFile.storageKey,
      sourceUrl: htmlFile.url,
      metadata,
    });
  ...
  async updateDouyinDirectVideo(brandId: string, workId: string, payload: UpdateDouyinDirectVideoPayload) {
    const target = await this.getVideoWorkRowById(brandId, workId);
    const meta = this.readVideoWorkMeta(this.getMediaMetadata(target));
    this.ensureVideoWorkKind(meta, "DOUYIN_DIRECT_VIDEO");
    const nextTitle = payload.title?.trim() || meta.title;
    const nextContent = payload.content?.trim() || meta.content;
    const copy = this.getVideoWorkKindCopy(meta.kind);
    const nextHtmlContent = this.renderGeneratedVideoNoteHtml({
    ...
    const nextMeta: VideoWorkAssetMeta = {
      ...
      htmlContent: nextHtmlContent,
      videoPrompt: nextContent,
      fullVideoPrompt: nextContent,
      creativeScript: undefined,
      updatedAt: new Date().toISOString(),
    };
    await this.writeGeneratedTextFile(brandId, this.extractFileName(target.storageKey || `${target.id}.html`), nextHtmlContent);
    await this.updateWorkHtmlMetadata(workId, brandId, nextMeta, `${copy.htmlTitlePrefix} - ${nextTitle}`);
  ...
  async continueDouyinDirectVideoGeneration(
    brandId: string,
    workId: string,
    payload: ContinueDouyinDirectVideoGenerationPayload,
    auth?: RequestAuthContext,
  ) {
    const target = await this.getVideoWorkRowById(brandId, workId);
    const meta = this.readVideoWorkMeta(this.getMediaMetadata(target));
    ...
    if (!meta.content?.trim()) {
      throw new BadRequestException("请先生成并确认生视频提示词，再继续生成短视频。");
    }
    const userId = await this.resolveTaskUserId(brandId, auth);
    const task = await this.createVideoTask({
      userId,
  ...
  async recoverDouyinDirectVideoGeneration(
    brandId: string,
    payload: RecoverDouyinDirectVideoGenerationPayload,
  ) {
    const providerTaskId = payload.providerTaskId?.trim();
    if (!providerTaskId) {
      throw new BadRequestException("请提供第三方视频任务 ID。");
    ...
    const target = payload.workId?.trim()
      ? await this.getVideoWorkRowById(brandId, payload.workId.trim())
      : await this.findRecoverableVideoWorkRow(brandId, providerTaskId, payload.requestedVideoProvider?.trim(), "DOUYIN_DIRECT_VIDEO");
    const workId = String(target.id || "").trim();
    const storageKey = target.storageKey || `${workId}.html`;
    const meta = this.readVideoWorkMeta(this.getMediaMetadata(target));
    this.ensureVideoWorkKind(meta, "DOUYIN_DIRECT_VIDEO");
    ...
    const nextMeta = await this.saveVideoWorkMetadataSnapshot(brandId, workId, storageKey, {
      ...
      videoAssetId: videoMedia.id,
      videoUrl: cachedVideoUrl,
      coverImageUrl: cachedCoverImageUrl || meta.coverImageUrl,
      progressSteps: this.buildVideoProgressSteps("SUCCESS", meta.kind),
    });
    await this.markTaskSuccess(
      taskId,
      { workId, stage: "VIDEO_RECOVERED", title: nextMeta.title, providerTaskId },
  ...
  async deleteDouyinDirectVideo(brandId: string, workId: string) {
    const target = await this.getVideoWorkRowById(brandId, workId);
    const meta = this.readVideoWorkMeta(this.getMediaMetadata(target));
    this.ensureVideoWorkKind(meta, "DOUYIN_DIRECT_VIDEO");
    const taskId = meta.taskId || target.taskId || undefined;
              ...
              try {
                ...
                if (provider.requestMode === "apiz-task") {
                  ...
                  const createResponse = await this.requestAuthorizedJson(baseUrl, provider.createPath, apiKey, {
                    method: "POST",
                    body: this.buildImageGenerationPayload(provider, modelName, promptCandidate, referenceImages, promptMode, params.imageSizeOverride),
                  ...
                  const response = await this.requestModelCompletion(
                    ...
                    provider.completionPath,
                    apiKey,
                    this.buildImageGenerationPayload(provider, modelName, promptCandidate, referenceImages, promptMode, params.imageSizeOverride),
  ...
  private async updateTaskRunningOutput(
    taskId: string,
  ...
  private normalizeWechatBodyImageSizeValue(value: unknown): WechatBodyImageSize {
    const normalized = String(value || "").trim();
    if (
      normalized === "landscape-16-9"
      || normalized === "square-1-1"
      || normalized === "portrait-4-3"
    ) {
      return normalized;
    }
  ...
  private buildVideoProgressSteps(stage: VideoWorkflowStage, kind: VideoWorkKind = "XHS_VIDEO_NOTE"): VideoProgressStepEntry[] {
    ...
    if (kind === "DOUYIN_DIRECT_VIDEO") {
      const mapStatus = (key: "SCRIPT" | "VIDEO"): VideoProgressStepEntry["status"] => {
        ...
        if (stage === "WAITING_VIDEO") {
          return key === "SCRIPT" ? "SUCCESS" : "PENDING";
        }
        if (stage === "GENERATING_VIDEO") {
          return key === "VIDEO" ? "RUNNING" : "SUCCESS";
        }
      ...
      return [
        { key: "SCRIPT", label: "生视频提示词", status: mapStatus("SCRIPT") },
  ...
  private getVideoWorkKindCopy(kind: VideoWorkKind) {
    ...
    if (kind === "DOUYIN_DIRECT_VIDEO") {
      return {
        taskLabel: "AI 生视频",
        generatingLabel: "AI 生视频生成中",
        noteLabel: "抖音 AI 生视频",
        htmlTitlePrefix: "抖音 AI 生视频",
        videoAssetTitlePrefix: "AI 生视频视频",
      };
    }
    return {
      taskLabel: "视频笔记",
  ...
  private getVideoSkillProfile(kind: VideoWorkKind) {
    ...
    if (kind === "DOUYIN_DIRECT_VIDEO") {
      return {
        skillSlug: "douyin-direct-video-studio",
        promptId: "prompt_douyin_direct_video",
        brandScriptPromptId: "prompt_douyin_direct_video",
        spokenScriptPromptId: "prompt_douyin_direct_video",
        skitScriptPromptId: "prompt_douyin_direct_video",
        remixScriptPromptId: "prompt_douyin_direct_video",
        storyboardPromptId: "prompt_douyin_direct_video",
        skillLabel: "抖音 AI 生视频",
      } as const;
    }
    return {
      skillSlug: "short-video-api-studio",
  ...
  private async resolveDouyinDirectVideoComposerContext(
    brandId: string,
    payload: GenerateDouyinDirectVideoPayload,
    collaboratorRole: "ADMIN" | "STAFF" | "TALENT",
  ): Promise<ResolvedVideoComposerContext> {
    ...
    const latestMarketingPlan = marketingPlanWorkspace.latest;
    if (includeMarketingPlan && !latestMarketingPlan) {
      throw new BadRequestException("请先生成抖音营销策划方案，再创作 AI 生视频。");
    }
    const calendarWorkspace = await this.reportsService.getXiaohongshuMarketingCalendarWorkspace(brandId);
    const selectedCalendarItem = this.findSelectedCalendarItem(calendarWorkspace.history, payload.calendarItemId);
    const customTopicName = payload.customTopicName?.trim() || undefined;
    ...
    const referenceImageUrl = payload.referenceImage?.dataBase64
      ? (
        await this.persistUploadFile(
          brandId,
          `${randomUUID()}-douyin-direct-video-reference${this.resolveExtensionFromFileName(payload.referenceImage.fileName, ".png")}`,
          payload.referenceImage,
        )
      ).url
      : undefined;
    ...
    return {
      ...
      requestedVideoProvider,
      requestedDurationSec: this.normalizeRequestedVideoDuration(payload.durationSec),
      requestedAspectRatio: this.normalizeVideoAspectRatio(payload.aspectRatio),
      copyAdditionalInstruction: additionalInstruction,
      videoAdditionalInstruction: additionalInstruction,
    };
  }
  ...
  private async runInitialDirectVideoWorkflowTask(
    brandId: string,
    workId: string,
    taskId: string,
    context: ResolvedVideoComposerContext,
    ...
    try {
      ...
      let meta = await this.saveVideoWorkMetadataSnapshot(brandId, workId, storageKey, {
        ...
        taskId,
        workflowStage: "GENERATING_SCRIPT",
        progressSteps: this.buildVideoProgressSteps("GENERATING_SCRIPT", "DOUYIN_DIRECT_VIDEO"),
      });
      const promptResult = await this.generateDouyinDirectVideoPrompt(brandId, context);
      meta = await this.saveVideoWorkMetadataSnapshot(brandId, workId, storageKey, {
        ...meta,
        taskId,
        ...
        scriptModel: promptResult.modelName,
        videoPrompt: promptResult.videoPrompt,
        fullVideoPrompt: promptResult.fullVideoPrompt,
        workflowStage: "WAITING_VIDEO",
        progressSteps: this.buildVideoProgressSteps("WAITING_VIDEO", "DOUYIN_DIRECT_VIDEO"),
      });
      await this.updateTaskOutputJson(taskId, {
        workId,
        stage: "PROMPT_READY",
  ...
  private async runContinueVideoGenerationTask(
    ...
    try {
      ...
      let meta = await this.saveVideoWorkMetadataSnapshot(brandId, workId, storageKey, {
        ...
        taskId,
        workflowStage: "GENERATING_VIDEO",
        progressSteps: this.buildVideoProgressSteps("GENERATING_VIDEO", this.readVideoWorkMeta(this.getMediaMetadata(target)).kind),
      });
      ...
      const accessibleReferenceImageUrl = await this.resolveThirdPartyAccessibleAssetUrl(meta.storyboardImageUrl || meta.referenceImageUrl, brandId);
      ...
      const videoResult = await this.generateVideoAsset({
        ...
        requestedAspectRatio: meta.requestedAspectRatio,
        referenceImageUrl: accessibleReferenceImageUrl,
  ...
  private async generateDouyinDirectVideoPrompt(
    brandId: string,
    context: ResolvedVideoComposerContext,
  ): Promise<{
    title: string;
    ...
    const skillProfile = this.getVideoSkillProfile(context.workKind);
    const fallbackPrompt = await this.loadDouyinDirectVideoSkillPrompt();
    const result = await this.requestVideoStageJson({
      brandId,
      workKind: context.workKind,
      promptId: skillProfile.promptId,
      ...
      systemInstruction: [
        ...
        "JSON 结构固定为：",
        '{ "title": "作品标题", "content": "完整的 Seedance 2.0 生视频提示词", "video_prompt": "精简版提示词", "full_video_prompt": "完整提示词", "hashtags": ["标签"], "business_scene": "商业场景", "video_type": "视频类型" }',
      ].join("\n"),
      inputPayload: {
        topic: context.topicLabel,
        accountRole: context.accountRole,
        videoKind: this.getVideoKindLabel(context.videoKind),
        durationSec: context.requestedDurationSec,
        aspectRatio: context.requestedAspectRatio,
        marketingPlanMarkdown: this.buildVideoMarketingPlanContext(context.marketingPlanMarkdown),
        calendar: context.selectedCalendarItem || null,
        product: context.product || null,
        sourceMaterial: context.material || null,
        referenceImageUrl: context.referenceImageUrl || context.product?.imageUrl || null,
        additionalInstruction: context.videoAdditionalInstruction || context.copyAdditionalInstruction || null,
      },
    });
    const videoPrompt = String(
      result.parsed.video_prompt
      ?? result.parsed.videoPrompt
      ?? result.parsed.prompt
      ?? result.parsed.content
      ?? "",
    ).trim();
    const fullVideoPrompt = String(
      result.parsed.full_video_prompt
      ?? result.parsed.fullVideoPrompt
      ?? result.parsed.content
      ?? videoPrompt,
    ).trim() || videoPrompt;
    const content = fullVideoPrompt || videoPrompt;
    if (!content) {
      throw new ServiceUnavailableException("未生成有效的 Seedance 2.0 生视频提示词，请稍后重试。");
    }
    return {
      title: String(result.parsed.title ?? "").trim() || `${context.topicLabel} 生视频`,
      content,
      hashtags: this.normalizeStringArray(result.parsed.hashtags, [], 8),
      modelName: result.modelName,
      businessScene: this.readOptionalString(result.parsed.business_scene ?? result.parsed.businessScene),
      videoType: this.readOptionalString(result.parsed.video_type ?? result.parsed.videoType),
      videoPrompt: videoPrompt || content,
      fullVideoPrompt: fullVideoPrompt || content,
    };
  }
  ...
  private async generateDouyinRemixShortVideoAnalysis(
    ...
    const result = await this.requestVideoStageJson({
      ...
      fallbackPrompt: "先客观拆解原视频，再按原视频的呈现方式改写成品牌产品版复刻分镜包。",
      stageLabel: "抖音复刻短视频拉片分析",
      includeKnowledgeContext: context.injectBrandProfile,
      systemInstruction: [
        "请仅输出 JSON 对象，不要输出 Markdown。",
        "将视频按每 15 秒切成一段；如果最后不足 15 秒，也要保留最后一段。",
        "先做“原视频客观拆解”，再做“品牌版改写输出”，两者不能混淆。",
        "analysis_report 只能描述原视频真实发生了什么：人物、场景、镜头、节奏、字幕、台词、动作、景别、运镜、转场、道具与情绪推进；不允许把品牌产品、品牌口播、营销卖点写进 analysis_report。",
        "role_card_text、storyboard_script、role_image_prompt、storyboard_image_prompt、video_prompt 必须是“品牌版复刻结果”：在尽量保留原视频镜头结构、节奏、景别、场景关系、动作骨架和呈现方式的前提下，把原视频里的产品、文案、口播、卖点、字幕替换为当前品牌/产品相关内容。",
        "如果提供了品牌资料、产品资料、营销策划方案或用户要求，就必须优先用这些资料完成品牌版改写；不要继续沿用原视频中的原始产品名、原始卖点和原始口播。",
        "如果原视频是口播/讲解型视频，storyboard_script 必须沿用原视频的镜头方式与节奏，但把文案改写成品牌相关表达；不要凭空改成完全不同的剧情、场景或拍法。",
        "不允许虚构与原视频明显无关的新场景、新人物关系、新叙事主线；品牌版改写应当是“借原视频表现形式，重做品牌内容”，而不是另写一条新广告。",
        "JSON 结构固定为：",
  ...
  private readVideoWorkMeta(metadataJson: unknown): VideoWorkAssetMeta {
    ...
    return {
      ...
      requestedAspectRatio: this.normalizeVideoAspectRatio(this.readOptionalString(meta.requestedAspectRatio)),
...
<html lang="zh-CN">
  ...
  private async loadDigitalHumanScriptTemplates(userId: string, brandId: string) {
    if (await this.prismaService.canUseDatabase()) {
      try {
        ...
        const supportsNote = await this.hasDigitalHumanScriptTemplateColumn("note");
        const rows = await this.prismaService.$queryRawUnsafe<Array<{
          id: string;
          userId: string;
          name: string;
          ...
          supportsSharedTemplates
            ? `SELECT "id", "userId", "name", "content",
                     ${supportsNote ? `"note"` : `'' AS "note"`},
                     "isShared",
  ...
  private async createDigitalHumanScriptTemplate(
    ...
    if (await this.prismaService.canUseDatabase()) {
      try {
        ...
        if (payload.note && !supportsNote) {
          throw new ServiceUnavailableException("当前数据库尚未应用数字人模板备注迁移，请先执行最新 Prisma migration。");
        }
        const rows = supportsSharedTemplates
          ? await this.prismaService.$queryRawUnsafe<Array<{
            ...
            `INSERT INTO "digital_human_script_templates" ("id", "userId", "brandId", "name", "content", ${supportsNote ? `"note", ` : ""}"isShared", ${supportsCategory ? `"category", ` : ""}${supportsArchived ? `"isArchived", ` : ""}"createdAt", "updatedAt")
             VALUES ($1, $2, $3, $4, $5, ${supportsNote ? `$6, ` : ""}$${supportsNote ? "7" : "6"}, ${supportsCategory ? `$${supportsNote ? "8" : "7"}, ` : ""}${supportsArchived ? `$${supportsNote ? (supportsCategory ? "9" : "8") : (supportsCategory ? "8" : "7")}, ` : ""}CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
             RETURNING "id", "userId", "name", "content", ${supportsNote ? `"note"` : `'' AS "note"`}, "isShared", ${supportsCategory ? `"category"` : `'general' AS "category"`}, ${supportsArchived ? `"isArchived"` : `FALSE AS "isArchived"`}, "createdAt", "updatedAt"`,
            randomUUID(),
            userId,
            brandId,
            payload.name,
            payload.content,
            ...(supportsNote ? [payload.note] : []),
            payload.isShared,
          ...
          : await this.prismaService.$queryRawUnsafe<Array<{
            ...
            `INSERT INTO "digital_human_script_templates" ("id", "userId", "brandId", "name", "content", "createdAt", "updatedAt")
             ...
             RETURNING "id", "userId", "name", "content", '' AS "note", FALSE AS "isShared", ${supportsCategory ? `"category"` : `'general' AS "category"`}, FALSE AS "isArchived", "createdAt", "updatedAt"`,
            randomUUID(),
            userId,
            brandId,
            payload.name,
  ...
  private async updateDigitalHumanScriptTemplate(
    ...
    if (await this.prismaService.canUseDatabase()) {
      try {
        ...
        if (typeof payload.note !== "undefined" && !supportsNote) {
          throw new ServiceUnavailableException("当前数据库尚未应用数字人模板备注迁移，请先执行最新 Prisma migration。");
        }
        const rows = supportsSharedTemplates
          ? await this.prismaService.$queryRawUnsafe<Array<{
            ...
            `UPDATE "digital_human_script_templates"
             SET "name" = $4, "content" = $5, ${supportsNote ? `"note" = $6, ` : ""}"isShared" = $${supportsNote ? "7" : "6"}, ${supportsCategory ? `"category" = $${supportsNote ? "8" : "7"}, ` : ""}${supportsArchived ? `"isArchived" = $${supportsNote ? (supportsCategory ? "9" : "8") : (supportsCategory ? "8" : "7")}, ` : ""}"updatedAt" = CURRENT_TIMESTAMP
  ...
  private normalizeDigitalHumanScriptTemplateNote(value?: string) {
  ...
  private mapDigitalHumanScriptTemplateRow(item: {
    ...
    return {
      ...
      note: this.normalizeDigitalHumanScriptTemplateNote(item.note),
  ...
  private async loadDouyinDirectVideoSkillPrompt() {
    const prompt = await this.skillsPromptsService.getActivePromptById("prompt_douyin_direct_video");
    const skillPrompt = prompt?.content?.trim() || this.readFirstExistingTextFromCandidates([
      resolve(this.resolveWorkspaceRoot(), ".runtime", "prompt_extract", "douyin-direct-video-studio", "SKILL.md"),
      resolve(this.resolveAiWorkspaceRoot(), ".runtime", "prompt_extract", "douyin-direct-video-studio", "SKILL.md"),
      resolve(this.resolveAiWorkspaceRoot(), "提示词", "抖音板块", "AI生成视频不带故事板.txt"),
      resolve(this.resolveOperationRoot(), "提示词", "抖音板块", "AI生成视频不带故事板.txt"),
    ]);
    if (skillPrompt) {
      return skillPrompt;
    }
    return [
      "你是一位抖音短视频导演和 Seedance 2.0 提示词专家。",
      "请根据输入的选题、产品、素材、参考图、营销策划和用户要求，生成可直接用于 Seedance 2.0 的高质量视频提示词。",
      "输出只保留最终提示词正文与必要的负面约束，不要附加解释。",
    ].join("\n");
  }
  ...
  private buildDirectVideoPrompt(meta: VideoWorkAssetMeta) {
    if (meta.kind === "DOUYIN_DIRECT_VIDEO") {
      return String(meta.content || meta.fullVideoPrompt || meta.videoPrompt || "").trim();
    }
  ...
  private normalizeRequestedVideoDuration(value?: number) {
    if (value === 3 || value === 5 || value === 8 || value === 10 || value === 15) {
      return value;
    }
  ...
  private normalizeVideoAspectRatio(value?: string): VideoAspectRatio {
    if (value === "3:4" || value === "16:9" || value === "4:3" || value === "9:16") {
      return value;
    }
    return "9:16";
  }
      ...
      case "volcengine_seedance": {
        ...
        return {
          payload: {
            ...
            ratio: hasReferenceImage ? "adaptive" : (params.requestedAspectRatio || "9:16"),
            generate_audio: true,
            watermark: false,
        ...
        return {
          payload: buildApizTaskPayload(
            params.config.taskModel || "ark/seedance-2.0",
            {
              ...
              ...(hasReferenceImage
                ? { image_url: requireReferenceImage(), ratio: "adaptive" }
                : { ratio: params.requestedAspectRatio || "9:16", generate_audio: true }),
            },
          ),
          createPath: params.config.createPath,
          queryPath: params.config.queryPath,
  ...
  private buildImageGenerationPayload(
    ...
    if (provider.requestMode === "apiz-task") {
      ...
      const params: Record<string, unknown> = {
        prompt,
        image_size: imageSizeOverride?.apiz || this.resolveApizImageSize(modelName, referenceImageUrls, promptMode),
        resolution: "1K",
        num_images: 1,
      };
