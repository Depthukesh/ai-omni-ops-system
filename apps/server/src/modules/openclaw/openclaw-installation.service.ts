...
export class OpenClawInstallationService {
    ...
    return {
      ...
      skillGuide: {
        ...
        examples: [
          ...
          "帮我看当前品牌成员、邀请链接和权限配置",
          "帮我看 OpenClaw 的每日计划、每周复盘、策略优化记录、创作素材、视频作品、评论获客和平台获客名单",
      ...
      openChatCutGuide: {
        ...
        steps: [
          "先在独立目录检出 OpenChatCut 受控 fork 源码，并准备独立的持久化目录。",
          "不要把 OpenChatCut 整仓复制进本站主仓库；主仓库只保留部署样板、网关与文档，OpenChatCut 本体单独维护。",
          ...
          "如果 OpenChatCut 跑在 Docker、但本站主系统跑在宿主机，OpenChatCut 页面里的 API URL 不要填 `127.0.0.1:13011`；应改填 `host.docker.internal:13011` 或其它宿主机可达地址。",
          "最后让 OpenClaw 同时绑定本站 MCP 与 OpenChatCut MCP，按“本站产素材 -> OpenChatCut 剪辑导出”闭环联调。",
        ],
        env: [
          "`OPENCHATCUT_SOURCE_DIR`：OpenChatCut 受控 fork 的源码检出目录，建议与本站仓库平级放置。",
          ...
          "`OPENCHATCUT_TRUST_DOCKER_LOCALHOST=1`：允许 Docker 端口发布场景下的 `localhost/127.0.0.1` 浏览器请求通过 OpenChatCut 的本地来源校验，解决设置页测试接口常见的 `invalid request origin`。",
          "`OPENCHATCUT_EDITOR_URL`：OpenChatCut 编辑器对外地址，建议与浏览器真实访问地址保持一致，不要一边用 `localhost` 一边写成 `127.0.0.1`。",
        ...
        notes: [
          ...
          "如果某个修复必须改 OpenChatCut 本体，不要在每台电脑上手工改官方源码；要么维护一份受控 fork，要么维护我们自己的镜像，再让安装说明统一指向受控版本。",
          "受控 fork 并不代表和 upstream 断开；后续应继续按 `官方仓库 = upstream / 我们的 fork = origin` 小步同步原版更新，而不是长期分叉失控。",
          "当前受控 fork 已补 `OPENCHATCUT_TRUST_DOCKER_LOCALHOST` 开关，用于 Docker 端口发布场景下放行 `localhost/127.0.0.1` 浏览器请求；如果后续继续遇到 `invalid request origin`，优先先检查这个环境变量是否真的进了容器。",
          "如果测试接口报 `connect ECONNREFUSED 127.0.0.1:13011`，通常不是 Key 错，而是 OpenChatCut 容器正在请求它自己的 `127.0.0.1`；这时应把 OpenChatCut 页面里的 API URL 改成宿主机可达地址，例如 `host.docker.internal:13011`。",
    ...
    return [
      {
        ...
        lines: [
          `API URL = ${input.gatewayBaseUrl}/v1`,
          "API Key = OpenClaw 安装令牌（ocp_ 开头）",
          "接口格式 = Chat Completions API",
          "模型 = gpt-5.5 或 claude-sonnet-5 或 gemini-3.7-flash",
        ],
      },
      {
        title: "B1 类：生图 / 图生图",
  ...
  private buildOpenChatCutTutorialSections(input: {
    mcpServerName: string;
    mcpUrl: string;
    headerValue: string;
    brandId: string;
    ...
    const workbuddySnippet = JSON.stringify({
      mcpServers: {
        [input.mcpServerName]: {
          url: input.mcpUrl,
          headers: {
            Authorization: input.headerValue,
            "x-brand-id": input.brandId,
          },
          type: "streamableHttp",
          timeout: 600000,
        ...
        openchatcut: {
          url: "http://127.0.0.1:15199/api/external-mcp/mcp",
          headers: {
            Authorization: "Bearer <把 docker/openchatcut.env 里的 OPENCHATCUT_MCP_TOKEN 填到这里>",
          },
          type: "streamableHttp",
          timeout: 600000,
        },
    ...
    return [
      {
        title: "1. 拉取源码并准备目录",
        summary: "OpenChatCut 建议放在本站仓库平级目录。主仓库只保留部署样板和文档，不直接承接 OpenChatCut 整仓源码。",
        lines: [
          "# 在 local-ai-omni-ops-system 仓库根目录执行",
          "git clone <你的 OpenChatCut 受控 fork 地址> \"..\\OpenChatCut\"",
          "New-Item -ItemType Directory -Force -Path \".\\docker\\local-data\\openchatcut\\home\" | Out-Null",
          "New-Item -ItemType Directory -Force -Path \".\\docker\\local-data\\openchatcut\\media\" | Out-Null",
        ],
      },
      {
        title: "2. 生成 Token 并填写 openchatcut.env",
        summary: "WorkBuddy 连接 OpenChatCut MCP 时，Bearer Token 要取自 `docker/openchatcut.env` 里的 `OPENCHATCUT_MCP_TOKEN`，不要去读宿主机 `~/.openchatcut/mcp-token`。",
        lines: [
          "# 在本站仓库根目录执行",
          "Copy-Item \".\\docker\\openchatcut.env.example\" \".\\docker\\openchatcut.env\" -Force",
          "$openchatcutMcpToken = -join ((48..57) + (65..90) + (97..122) | Get-Random -Count 48 | ForEach-Object { [char]$_ })",
          "$resourcePreviewToken = -join ((48..57) + (65..90) + (97..122) | Get-Random -Count 48 | ForEach-Object { [char]$_ })",
          "notepad \".\\docker\\openchatcut.env\"",
          "",
          "OPENCHATCUT_SOURCE_DIR=../OpenChatCut",
          "OPENCHATCUT_HTTP_PORT=15199",
          "OPENCHATCUT_HOME_DIR=./local-data/openchatcut/home",
          "OPENCHATCUT_MEDIA_DIR=./local-data/openchatcut/media",
          "OPENCHATCUT_MCP_TOKEN=<把 $openchatcutMcpToken 生成的值填进来>",
          "OPENCHATCUT_TRUST_DOCKER_LOCALHOST=1",
          "OPENCHATCUT_EDITOR_URL=http://localhost:15199",
          "RESOURCE_PREVIEW_TOKEN=<把 $resourcePreviewToken 生成的值填进来>",
        ],
      },
      {
        title: "3. 启动与更新 OpenChatCut Docker",
        summary: "首次启动会更慢，因为容器里会先安装依赖并同步 MediaPipe 和 whisper-cli。以后更新要分清楚两件事：OpenChatCut 源码更新在 OpenChatCut 仓库做，Docker 重建在本站仓库做。",
        lines: [
          "# 首次启动 / 重建",
          "docker compose --env-file \"docker/openchatcut.env\" -f \"docker/docker-compose.openchatcut.yml\" up -d --build",
          "",
          "# 查看启动日志",
          "docker compose --env-file \"docker/openchatcut.env\" -f \"docker/docker-compose.openchatcut.yml\" logs -f openchatcut",
          "",
          "# 更新 OpenChatCut fork 源码（在 ..\\OpenChatCut 仓库执行）",
          "git pull --ff-only",
          "",
          "# 更新后回到本站仓库重新构建容器",
          "docker compose --env-file \"docker/openchatcut.env\" -f \"docker/docker-compose.openchatcut.yml\" up -d --build",
      ...
      {
        title: "4. WorkBuddy 双 MCP 配置",
        summary: "同一份 `mcp.json` 同时挂本站 MCP 和 OpenChatCut MCP。OpenChatCut 那条的 Bearer Token 必须来自 `docker/openchatcut.env` 的 `OPENCHATCUT_MCP_TOKEN`。",
        lines: workbuddySnippet.split("\n"),
      },
      {
        title: "5. OpenChatCut 页面填写与联调顺序",
        summary: "如果 OpenChatCut 跑在 Docker、但本站主系统跑在宿主机，OpenChatCut 页面里填 API URL 时不要写 `127.0.0.1:13011`，应改填 `host.docker.internal:13011`。Agent 大脑页必须带 `/v1`。",
        lines: [
          `A 类 Agent 大脑：API URL = ${input.gatewayBaseUrl}/v1`,
          "A 类 Agent 大脑：API Key = OpenClaw 安装令牌（ocp_ 开头）",
          "A 类 Agent 大脑：接口格式 = Chat Completions API",
          "A 类 Agent 大脑：模型 = gpt-5.5",
          "",
          `B1 生图 OpenAI：Base URL = ${input.gatewayBaseUrl}`,
          "B1 生图 OpenAI：API Key = OpenClaw 安装令牌（ocp_ 开头）",
          "B1 生图 OpenAI：OpenAI 页面没有模型输入框，默认模型就是 gpt-image-2",
          "",
          `B2 配音 / 转写：Base URL = ${input.gatewayBaseUrl}`,
          "B2 配音模型 = tts-1 或 tts-1-hd；转写模型 = whisper-1",
          "",
          `C 类视频 / 音乐：Base URL = ${input.gatewayBaseUrl}`,
          "C 类视频模型 = omni-fast；音乐模型 = chirp-v4",
        ],
      },
      {
        title: "6. 正确验证顺序",
...
- 团队成员、邀请链接、权限模板、品牌协作
- OpenClaw 每日计划、每周复盘、策略优化记录、创作素材、视频作品、音乐任务
...
- 投流获客腾讯投流获客列表，以及标题 / 正文内容的写入、详情留言与删除
...
- 帮我看 OpenClaw 的每日计划、每周复盘、策略优化记录和创作素材
- 帮我保存一份 GEO 可见度诊断报告，并告诉我现在已经有几份报告
- 帮我把小红书和抖音评论用户生成评论获客列表
- 帮我把这批平台名单写入平台获客
- 帮我把一条腾讯投流获客内容写入投流获客工作台
