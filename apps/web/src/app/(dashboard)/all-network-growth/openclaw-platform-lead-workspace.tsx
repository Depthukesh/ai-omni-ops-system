...
import { type OpenClawPlatformLeadRecord } from "../../../services/openclaw";

const PAGE_SIZE = 20;

type OptionalDateFormatter = (value?: string) => string;

export interface OpenClawPlatformLeadWorkspaceProps {
  sectionLabel: string;
...
export function OpenClawPlatformLeadWorkspace(props: OpenClawPlatformLeadWorkspaceProps) {
  const [selectedLead, setSelectedLead] = useState<OpenClawPlatformLeadRecord | null>(null);
  const [currentPage, setCurrentPage] = useState(1);

  const sortedItems = useMemo(
    () => [...props.items].sort((left, right) => right.selectedAt.localeCompare(left.selectedAt)),
    [props.items],
  );
      ...
      <article className="workspace-panel strategy-page-card">
        ...
        {!sortedItems.length ? (
          <div className="note-empty-state">
            当前还没有平台获客记录。请先由 OpenClaw 把平台名单写入这里。
          </div>
        ) : (
          <>
            <div className="note-pagination-bar" style={{ marginBottom: 12 }}>
              ...
              <div className="note-pagination-actions">
                ...
                {Array.from({ length: totalPages }, (_, index) => index + 1)
                  ...
                  .map((pageNumber) => (
                    <button
                      key={`platform-lead-page-${pageNumber}`}
                      type="button"
                  ...
                  {pagedItems.map((item) => (
                    <tr key={item.id}>
                      <td className="openclaw-record-table__text-cell">
                        <span className="openclaw-record-table__text" title={item.name}>{item.name || "-"}</span>
                      </td>
                      <td className="openclaw-record-table__text-cell">
                        <span className="openclaw-record-table__text" title={item.businessScope}>{item.businessScope || "-"}</span>
                      </td>
                      <td className="openclaw-record-table__text-cell">
                        <span className="openclaw-record-table__text" title={item.selectedReason}>{item.selectedReason || "-"}</span>
                      </td>
                      <td className="openclaw-record-table__text-cell">
                        <span className="openclaw-record-table__text" title={item.contactInfo}>{item.contactInfo || "-"}</span>
                      </td>
                      <td className="openclaw-record-table__text-cell">
                        <span className="openclaw-record-table__text" title={item.address}>{item.address || "-"}</span>
                      </td>
        ...
        <div className="openclaw-diary-dialog-backdrop" onClick={() => setSelectedLead(null)}>
          <div className="openclaw-diary-dialog" onClick={(event) => event.stopPropagation()} style={{ width: "min(980px, 94vw)" }}>
            <div className="openclaw-diary-dialog__head">
              <div>
                <strong>{selectedLead.name || "平台获客详情"}</strong>
                <p>入选时间 {props.formatDateTime(selectedLead.selectedAt)}</p>
              </div>
            ...
            <div className="openclaw-diary-dialog__meta">
              <span>联系方式：{selectedLead.contactInfo || "-"}</span>
            </div>
            <div className="openclaw-diary-dialog__meta">
              <span>地址：{selectedLead.address || "-"}</span>
            </div>
          </div>
        </div>
      ) : null}
