...
function buildLauncherCmd() {
  return [
    ...
    'pushd "%APP_DIR%" >> "%START_LOG%" 2>&1',
    "if errorlevel 1 (",
    '  echo Failed to enter APP_DIR >> "%START_LOG%"',
    "  exit /b 1",
    ")",
    '"%NODE_EXE%" "scripts\\local-single-user-launcher.cjs" >> "%START_LOG%" 2>&1',
...
function buildInstallPowerShellScript() {
  return [
    ...
    "  }",
    "",
    "  Write-Host (\"Stopping existing runtime PIDs: \" + (($uniquePids | ForEach-Object { [string]$_ }) -join ', '))",
    ...
    '  } catch {',
    '    throw ("Failed to move existing install to backup. InstallRoot={0}; BackupRoot={1}; Error={2}" -f $InstallRoot, $backupRoot, $_.Exception.Message)',
    '  }',
    '}',
    "",
    ...
    'Start-Process -FilePath $launchPath -WorkingDirectory $InstallRoot -WindowStyle Hidden',
    "",
...
function buildInstallCmd() {
  return [
    ...
    "setlocal",
    'set "SCRIPT_DIR=%~dp0"',
    'set "LOG_DIR=%LOCALAPPDATA%\\AiOmniOps\\logs"',
    'if not exist "%LOG_DIR%" mkdir "%LOG_DIR%" >nul 2>&1',
    ...
    "powershell -NoProfile -ExecutionPolicy Bypass -Command \"& { & '%INSTALL_PS1%' *>&1 | Tee-Object -FilePath '%INSTALL_LOG%' -Append }\"",
    "set EXIT_CODE=%ERRORLEVEL%",
    "if not \"%EXIT_CODE%\"==\"0\" (",
    '  echo.',
